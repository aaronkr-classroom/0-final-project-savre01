 ...
 * 사용자 모델에 가상 속성 추가
 */
gameSchema.virtual("fullName").get(function () {
...
gameSchema.pre("save", function (next) {
  let game = this; // 콜백에서 함수 키워드 사용
  if (game.subscribedAccount === undefined) {
    // 기존 Subscriber 연결을 위한 조건 체크 추가
    Subscriber.findOne({
      email: game.email,
    }) // Single Subscriber를 위한 퀴리
      .then((subscriber) => {
        game.subscribedAccount = subscriber; // 사용자와 구독자 계정 연결
        next();
      })
      .catch((error) => {
